...
const FamilyDetailsForm: React.FC<FamilyDetailsFormProps> = ({ onSubmit, onPrevious, defaultValues }) => {
  ...
  const [appendd,setAppenddd] = useState(0);
        ...
        <div key={member.id} className="family-details-row">
          <div className="form-group">
            ...
            {errors.familyMembers?.[index]?.firstName && (
              <p className="error">{errors.familyMembers[index]?.firstName?.message}</p>
            )}
          </div>
          <div className="form-group">
            ...
            {errors.familyMembers?.[index]?.lastName && (
              <p className="error">{errors.familyMembers[index]?.lastName?.message}</p>
            )}
          </div>
          <div className="form-group">
            ...
            {errors.familyMembers?.[index]?.relation && (
              <p className="error">{errors.familyMembers[index]?.relation?.message}</p>
            )}
          </div>
          <div className="form-group">
            ...
            <Controller
              ...
              rules={{ 
                ...
                validate: value => {
                  ...
                  return Number.isInteger(age) && age > 0 || 'Age must be a positive integer';
                }
              }}
              render={({ field }) => <input {...field} type="text" className="inp" />}
            />
            {errors.familyMembers?.[index]?.age && (
              <p className="error">{errors.familyMembers[index]?.age?.message}</p>
            )}
          </div>
          {index !== 0 && ( // Only show Remove button for fields other than the first one
            <button type="button" onClick={() => handleRemoveField(index)} className="remove-button">Remove</button>
          )}
          <hr />
        </div>
      ))}
      <button type="button" onClick={handleAddField} className="add-button">+ Add Family Member</button>
      <button type="submit" className="submit-button">Next</button>
      <button type="button" onClick={onPrevious} className="previous-button">Previous</button>
    </form>
  );
};
